...
export class SettingControl extends Listener {
  static init() {
    ...
    ["world", "stage", "difficulity"].forEach(_ => {
      super.$(_).value = localStorage.getItem(_) == undefined ? super.$(_).children[0].innerHTML : localStorage.getItem(_);
    ...
    super.$("StartButton").addEventListener('click', () => {
      ...
      super.$("scoreboard").innerText = globalThis.world + globalThis.stage > 2 ? `You've resumed the game from ${globalThis.world}-${globalThis.stage}!` : "You've started a new game. Good luck!";
      let op = 1;
      const timer = setInterval(() => {
        ...
        new Traitable(element["style"]).with({
          opacity: op,
          filter: 'alpha(opacity=' + op * 100 + ")"
        });
        op *= 0.6;
      }, 50);
      util.range(parseInt(super.$("title").style["margin-top"].slice(0, 2)), 20).forEach((_, i) => {
