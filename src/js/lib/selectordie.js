        ...
        var _private = {
                ...
                return this.each(function () {
                    if (!$(this).parent().hasClass('sod_select')) {
                        var $select = $(this),
                            ...
                            $settingsPlaceholderOption = $select.data(
                                'placeholder-option'
                            )
                            ...
                            $settingsLinks = $select.data('links')
                                ? $select.data('links')
                                : $_settings.links,
                            $settingsLinksExternal = $select.data(
                                'links-external'
                            )
                            ...
                            $selectTitle = $select.prop('title')
                                ? $select.prop('title')
                                : null,
                            $selectDisabled = $select.is(':disabled')
                                ? ' disabled'
                                : '',
                            $sodPrefix = '',
                            $sodHtml = '',
                        ...
                        $('option, optgroup', $select).each(function (i) {
                            ...
                            if ($settingsStripEmpty && !$.trim($this.text())) {
                                ...
                                i === 0 &&
                                $settingsPlaceholderOption &&
                                !$sodPrefix
                            )
                                _private.populateSoD(
                                    $this,
                                    $sodList,
                                    $sod,
                                    true
            ...
            populateSoD: function ($option, $sodList, $sod, $isPlaceholder) {
                var $sodPlaceholder = $sod.data('placeholder'),
                    ...
                    $optionCustomClass = $option.data('custom-class')
                        ...
                        : '',
                    $optionIsDisabled = $option.is(':disabled')
                        ? ' disabled '
                        : '',
            ...
            triggerSod: function (e) {
                ...
                if (
                    ...
                    if ($sodPlaceholder) {
                        $sod
                            .find('.sod_label')
                            .get(0).lastChild.nodeValue = $optionActive.text();
                    }
                }
            }, // triggerSod
            ...
            keyboardUse: function (e) {
                ...
                if (e.which > 36 && e.which < 41) {
                    ...
                    if ($optionNext.hasClass('sod_option') || $sodCycle) {
                        ...
                        $sodLabel.get(
                            0
                        ).lastChild.nodeValue = $optionNext.text();
                ...
                if (e.which !== 0) {
                    ...
                    if ($sodFilterHit.length) {
                        ...
                        $sodLabel.get(
                            0
                        ).lastChild.nodeValue = $sodFilterHit.text();
            ...
            optionClick: function (e) {
                ...
                if (!$optionDisabled && !$optionOptgroup) {
                    $sod.find('.selected, .sod_placeholder').removeClass(
            ...
            selectChange: function () {
                ...
                $sod
                    .find('.sod_label')
                    .get(0).lastChild.nodeValue = $optionText;
                $sod.data('label', $optionText);
            ...
            blurSod: function ($sod) {
                if ($('body').find($sod).length) {
                    ...
                    if (
                        $_sodKeysWhenClosed &&
                        !$optionActive.hasClass('selected')
                    ) {
                        $optionActive.click();
                    ...
                    if (!$optionHasChanged && $sodPlaceholder) {
                        $sod
                            ...
                            .get(
                                0
                            ).lastChild.nodeValue = $optionSelected.text();
                    } else if (!$optionHasChanged) {
                        $sod
                            .find('.sod_label')
                            .get(0).lastChild.nodeValue = $sodLabel;
                    }
            ...
            checkViewport: function ($sod, $sodList) {
                ...
                if (
                    $sodPosition.bottom + $sodListHeight + 10 >
                        $(window).height() &&
                ...
                return this.each(function () {
                    ...
                    if ($sod.hasClass('sod_select')) {
                        if (typeof $value !== 'undefined') {
                            ...
                            $sod.find(
                                ".sod_list:first .sod_option[data-value='" +
                                    $value +
                                    "']"
                            ).addClass('disabled');
                            $sod.find(
                                ".sod_list:first .sod_option[data-label='" +
                                    $value +
                                    "']"
                            )
